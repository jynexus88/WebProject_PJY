...
public class BoardPage {
	public static String pagingStr(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
		...
		/* 전체 페이지 수 계산
		 * ceil(전체 게시물 수 / 한 페이지 당 게시물 수) => 결과를 무조건 올림처리하여 계산한다. */
		int totalPages = (int) (Math.ceil(((double) totalCount / pageSize)));
		
		/* '이전 페이지 바로가기' 출력
		 * 페이지 블록에서 첫 번째 페이지 번호를 구하기 위한 계산식
		 * 우리는 blockPage를 5로 설정했으므로 어래 계산식에 pageNum=1~5를 대입해보면 1의 결과가 나온다.
		 * 즉, 1~5페이지 일때는 pageTemp가 1이 되어 반복하면서 출력할 수 있다.
		 * */
		int pageTemp = (((pageNum - 1) / blockPage) * blockPage) + 1;
		
		// 첫 번째 블록이 아닌 경우에만 이전 페이지로 바로가기 링크를 출력한다.
		if (pageTemp != 1) {
